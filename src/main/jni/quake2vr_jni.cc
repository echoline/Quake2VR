...
(JNIEnv* env, jobject obj, jobject asset_mgr, jstring g) {
  jboolean isCopy = 1;
  const char *s = env->GetStringUTFChars(g, &isCopy);
  jlong ret = jptr(new Quake2VR(javaVm, obj, asset_mgr, s));
  env->ReleaseStringUTFChars(g, s);
  return ret;
}
